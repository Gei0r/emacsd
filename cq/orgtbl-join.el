...
(defun orgtbl--row-contains-column-alignment (row)
  "Check whether given table row contains table width alignment
definitions and therefor is not the headline of the table."
  (catch :exit
    (dolist (cell row)
      (when (string-match "\\`<\\([lrc]\\)?\\([0-9]+\\)?>\\'" cell)
        (throw :exit t)))))

(defun orgtbl--get-headline (table)
  "Get headline of table if exists. Skip 'hline and width
alignment rows before headline. Return headline or nil."
  ;; Skip 'hline before headline
  (while (eq 'hline (car table))
    (setq table (cdr table)))
  (if (memq 'hline table) ;; table has a header
      (progn
        (while (orgtbl--row-contains-column-alignment (car table))
          (setq table (cdr table)))
        (car table))))

(defun orgtbl--split-table (table)
  "Retrieve the special rows of the lisp table. Return a cons of
table and a list of positions in the table list.

  (table list-of-positions)

  list-of-positions ::= (num-hlines-before-table
                         pos-column-width-definition or nil if not existent
                         pos-headline or nil if not exsistent
                         pos-body-start)."
  (let ((tbl table)
        (row 0)
        (hlines-before-table 0)
        pos-column-width-definition
        pos-headline
        pos-body
        )
    (while (not (listp (car tbl)))
      (setq tbl (cdr tbl)
            row (1+ row)
            hlines-before-table (1+ hlines-before-table)))
    (if (memq 'hline tbl) ;; table has headerline
        (progn
          (while (not (eq 'hline (car tbl)))
            (if (orgtbl--row-contains-column-alignment (car tbl))
                (setq pos-column-width-definition row))
            (setq pos-headline row)
            (setq tbl (cdr tbl))
            (incf row)
            )
          (incf row)
    ...
    (setq pos-body row)
    (cons table
          (list hlines-before-table pos-column-width-definition pos-headline pos-body))
    )
  )
...
$1, $2..."
  (while (eq 'hline (car table))
    (setq table (cdr table)))
  (let ((headline (orgtbl--get-headline table)))
...
hashtable."
  ...
  (let ((head (orgtbl--get-headline table))
...
different reference row.  If no matching row is found in the
reference table, then the current row is kept, with empty cells
appended to it. Rows from the reference table which are not
found in the current table are appended."
  (interactive)
  (org-table-check-inside-data-field)
  (let* ((col (1- (org-table-current-column)))
	 (tbl (org-table-to-lisp))
    ...
    (mapc (lambda (masline)
	    ...
	    (forward-line 1))
	  tbl)
    ;; now all rows from ref which where not used
    (maphash (lambda (key value)
               (unless (member key drows)
                 (open-line 1)
                 (insert "|")
                 (let ((i 0))
                   (while (< i cols)
...
;; (defun cq-tbl-full-outer-join ()
;;   ""
;;   (interactive)
;;   (org-table-check-inside-data-field)
;;   (let* ((col (1- (org-table-current-column)))
;; 	 (tbl (org-table-to-lisp))
;;          (ref-bf (read-buffer "Buffer B (older) for merge in: "
;;                               (progn
;;                                 ;; realign buffers so that two visible bufs will be
;;                                 ;; at the top
;;                                 (save-window-excursion (other-window 1))
;;                                 (ediff-other-buffer (buffer-name)))
;;                               t))
...
;;   (let ((ref-bf (read-buffer "Buffer A (newer) with column to merge: "
