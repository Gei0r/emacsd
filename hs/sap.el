...
(defcustom sap-browser
  "d:/bin/FirefoxPortable/App/Firefox64/firefox.exe"
...
;; SAP-ID
(defvar thing-at-point-sap-id-regexp
  "A6Z[0-9]\\{11\\}/[0-9A-Z]\\{3\\}/[0-9A-Z]\\{3\\}/[0-9A-Z-]+"
  "A regular expression probably matching a sap id.")

;; (defun x ()
;;   (interactive)
;;   (with-temp-buffer
;;     (insert "A6Z00035927624/PM2/000/-  ")
;;     (goto-char (point-min))
;;     ;;(search-forward-regexp "A6Z[0-9]\\{11\\}/")
;;     (search-forward-regexp thing-at-point-sap-id-regexp)
;;     (goto-char (point-min))
;;     (thing-at-point 'sap-id)
;;     )
;;   )

;; Haven't set 'forward-op on 'segmented-address nor defined
;; 'forward-segmented-address' because not sure they're actually needed, and
;; URL seems to skip them too.  Note that (end-of-thing 'segmented-address) and
;; (beginning-of-thing 'segmented-address) work automagically, though.

(put 'sap-id 'bounds-of-thing-at-point
     (lambda ()
       (let ((thing (thing-at-point-looking-at
                     thing-at-point-sap-id-regexp 30)))
         (if thing
             (let ((beginning (match-beginning 0))
                   (end (match-end 0)))
               (cons beginning end))))))

(put 'sap-id 'thing-at-point
     (lambda ()
       (let ((boundary-pair (bounds-of-thing-at-point 'sap-id)))
         (if boundary-pair
...
(defun sap-open-url (url)
  ...
  (cond
   ((and
     (fboundp 'browse-url)
     cq-browser)
    (let ((browse-url-firefox-program sap-browser)
...
(define-key sap-mouse-map [\C-c \C-i] 'sap-open-file-from-sap-id-ask)
...
(define-minor-mode sap-minor-mode
  "Toggle sap-minor-mode.
Toggle syntax highlighting for SAP-IDs in this buffer.
With a prefix argument ARG, enable Sap Minor Mode mode if ARG is
...
the mode if ARG is omitted or nil."
  ...
  (if sap-minor-mode
    ...
    ;; (remove-from-invisibility-spec 'continuation)
    ;; (kill-local-variable 'search-invisible)
    ;; (kill-local-variable 'isearch-invisible)
    )
  )
