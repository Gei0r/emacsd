/**
  ...
  Configuration for LSP (language server protocol) and clangd.
  ...
  2. Input: Source file name (e.g. *.cpp, *.h)
     Output (file): .clangd at project root

     node lspConfig.js <filename>

  All input is is through command line arguments.
  Output is through stdout or .clangd file.
    ...
    try {
        ...
        for (let file of jsFiles) {
            ...
            if (getRoot) {
                ...
                let res = await plugin.writeClangd(process.argv[2], logfile);
                if (res == undefined) continue;

                const resultJson = JSON.stringify(res, null, 2);
                fs.writeSync(logfile, resultJson + "\n");
